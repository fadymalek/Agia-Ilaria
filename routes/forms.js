...
router.get('/individual', (req, res) => res.render('forms/individual', { error: null }));
...
router.post('/individual', wrap(async (req, res) => {
  const d = req.body;
  try {
    if (!d.church_name) throw new Error('الاسم مطلوب');
    const booking_number = await generateBookingNumber();
    const paid = parseFloat(d.paid_amount) || 0;
    const total = parseFloat(d.total_amount) || 0;
    await bookings.insert({
      booking_number, booking_type: 'individual_retreat', status: 'pending', source: 'form',
      sector_name: d.sector_name || null, church_name: d.church_name,
      priest_name: d.priest_name || null, priest_phone: d.priest_phone || null,
      age_group: d.age_group || null, num_people: parseInt(d.num_people) || 1,
      num_days: parseInt(d.num_days) || null, start_date: d.start_date || null, end_date: d.end_date || null,
      floor_number: d.floor_number || null, total_amount: total, paid_amount: paid, remaining_amount: total - paid,
      signature_name: d.signature_name || null, created_at: new Date().toISOString(), updated_at: new Date().toISOString()
    });
    res.render('forms/success', { booking_number, type: 'individual_retreat' });
  } catch (e) { res.render('forms/individual', { error: e.message }); }
}));
